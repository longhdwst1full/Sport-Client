const GUEST_ORDER_ACCESS_KEY = 'baoan_guest_order_access_v1';
...
function persist(access: GuestOrderAccess): void {
  ...
  window.localStorage.setItem(GUEST_ORDER_ACCESS_KEY, JSON.stringify(access));
}

function readAll(): GuestOrderAccess {
  if (typeof window === 'undefined') return sessionAccess;
  try {
    const now = Date.now();
    const parsed: unknown = JSON.parse(window.localStorage.getItem(GUEST_ORDER_ACCESS_KEY) ?? '{}');
