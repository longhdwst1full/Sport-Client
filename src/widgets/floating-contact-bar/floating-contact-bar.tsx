  ...
  return (
    <div className="fixed bottom-4 right-3.5 z-50 flex flex-col items-end gap-2.5 pointer-events-none sm:bottom-6 sm:right-5 sm:gap-3">
      ...
      <div className="flex flex-col items-end gap-2 pointer-events-auto sm:gap-2.5">
        ...
        <Link
          href="/contact"
          className="group relative flex items-center gap-2.5 rounded-full bg-slate-900 p-2.5 sm:p-3 text-emerald-400 shadow-xl shadow-slate-900/40 transition-all duration-300 hover:bg-slate-800 hover:scale-110 active:scale-95"
      ...
      {showBackToTop && (
        <button
          type="button"
          onClick={scrollToTop}
          className="pointer-events-auto group relative flex items-center justify-center rounded-full border border-slate-200 bg-white p-2 text-slate-700 shadow-lg transition-all duration-300 hover:border-emerald-400 hover:bg-emerald-50 hover:text-emerald-700 hover:scale-105 active:scale-95 sm:p-2.5"
          aria-label="Cuộn lên đầu trang"
        >
          <ArrowUp className="size-4 sm:size-4.5 transition-transform duration-200 group-hover:-translate-y-0.5" />
        </button>
      )}
    </div>
  );
